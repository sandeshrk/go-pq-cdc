...
| `publication.pruneTables`               |   bool   |    no    | false | When true, treats `publication.tables` as the full authoritative table membership: any live publication member missing from it is dropped via `ALTER PUBLICATION ... SET TABLE`. Tables present in config but not yet live are NOT added automatically. | Destructive if `publication.tables` is a deliberate subset -- only enable if config always lists every table that should remain published. |
