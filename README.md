...
By default, an unexpected replication connection loss (network blip, PostgreSQL restart, `pg_terminate_backend`, etc.) stops the stream and `Run(ctx)` returns `replication.ErrStreamCorrupted`. This is deliberate: a replication slot that stops advancing prevents PostgreSQL from recycling WAL, so a silent, unbounded retry loop is not a safe default.

> **Behavior change**: before this library returned `Run(ctx) error`, an unrecoverable connection loss `panic`ked the whole process, relying on an external supervisor (Kubernetes, systemd, ...) to restart it. `Run`/`Supervise` callers are unaffected other than getting a clean error instead of a crash. A `Start(ctx)` caller (the error-discarding wrapper, kept for backward compatibility) will now see streaming stop with **no crash and no restart** -- the process stays alive, idle. Callers that relied on the old crash-and-get-restarted behavior should migrate to `Run(ctx)` or `cdc.Supervise`.

Setting `reconnect.enabled: true` makes the connector try to reopen the replication stream in-process first, following an exponential backoff (`reconnect.initialDelay`, capped at `reconnect.maxDelay`, with `reconnect.maxJitter` added to avoid a reconnect storm across a fleet). If the connection comes back before `reconnect.maxElapsed` (and `reconnect.maxAttempts`, if set) is exhausted, the stream resumes from the slot's last confirmed position exactly as it would on a fresh restart -- no messages are lost, though any unacknowledged transaction is redelivered. A permanent failure (e.g. a rotated-away credential) is detected and given up on immediately rather than retried for the whole budget. If the budget runs out, `Run(ctx)` still returns `replication.ErrStreamCorrupted`, same as with `reconnect.enabled: false`.
...
A few sentinels are plain stdlib errors specifically so `errors.Is` works on them directly, with no unwrapping needed: `cdc.ErrConnectorConsumed`, `replication.ErrorSlotInUse`, `replication.ErrorNotConnected`, and `replication.ErrStreamCorrupted`.
...
| `lsnFlushInterval`                      | Duration |    no    |    5s   | Periodically sends a standby status update on the replication connection, independent of read-idle timeouts or PostgreSQL's own keepalives. | Without it, `confirmed_flush_lsn` only advances when the sink's read loop goes idle or a keepalive requests a reply -- under sustained throughput (especially with `wal_sender_timeout` disabled) neither may happen, so WAL is retained indefinitely. Set to a negative value to disable. |
| `reconnect.enabled`                     |   bool   |    no    |  false  | Attempt an in-process reconnect of the replication stream after an unexpected disconnect, instead of giving up immediately. | Disabled by default: an unrecovered disconnect makes `Run(ctx)` return `replication.ErrStreamCorrupted`, exactly as before this option existed. See [Reconnect](#reconnect) below. |
...
| `reconnect.maxElapsed`                  | Duration |    no    |   5m    | Bounds the total wall-clock time spent reconnecting before giving up.                    | The meaningful ceiling: a replication slot that is not advancing prevents PostgreSQL from recycling WAL, so retrying forever is not a safe default. |
...
| `publication.tables[i].snapshotPartitionStrategy` |  string  |    no    | auto  | Override partition strategy for snapshot                                                              | **auto:** Auto-detect best strategy (sparse integer PKs fall back to `ctid_block`). **integer_range:** Sequential integer PKs. **ctid_block:** String/UUID/hash/sparse PKs. **offset:** Slow fallback. |
...
| `publication.tables[i].publicationFilter` |  string  |    no    |    -    | Native Postgres publication row filter (`CREATE`/`ALTER PUBLICATION ... WHERE (...)`). Unlike `queryCondition`, this DOES filter the live CDC/replication stream, not just the initial snapshot. Requires PostgreSQL 15+. | Example: `"status = 'active'"`. Cannot be combined with `replicaIdentity: NOTHING` (Postgres needs the old row to evaluate the filter on UPDATE/DELETE). When `createIfNotExists` is `false` and the publication already exists with a different filter, the connector reconciles it via `ALTER PUBLICATION ... SET TABLE`. Set to `"-"` (`publication.ClearPublicationFilter`) to remove an existing live filter -- an empty/omitted value means "not configured, leave the live filter alone", not "clear it". |
| `publication.pruneTables`               |   bool   |    no    | false | When true, treats `publication.tables` as the full authoritative table membership: any live publication member missing from it is dropped via `ALTER PUBLICATION ... SET TABLE`. | Destructive if `publication.tables` is a deliberate subset -- only enable if config always lists every table that should remain published. Tables present in config but not yet live are always added regardless of this flag (see below). |
...
| go_pq_cdc_stream_unacked_lsn_lag                    | The highest WAL position received by this process minus the highest position acked by the consumer. Distinguishes a slow/stalled consumer from a slot that simply hasn't been flushed yet. | slot_name, host| Gauge      |
| go_pq_cdc_stream_wal_backlog_warning_total           | Incremented whenever WAL keeps arriving but nothing has been acked for several consecutive flush intervals (`lsnFlushInterval`), which prevents the slot from advancing. See the log warning of the same condition for remediation (typically: enable `heartbeat`). | slot_name, host| Counter    |
| go_pq_cdc_reconnect_replayed_messages_total          | Total number of messages redelivered after an in-process reconnect because they were already delivered (but not necessarily acked) before the disconnect. Measures the size of the duplicate window described in [Delivery Guarantees](#delivery-guarantees). | slot_name, host| Counter    |
